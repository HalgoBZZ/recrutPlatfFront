...
export class CandidatLangueService {

  private uri = 'http://localhost:8085/api/candidatLangue/';
  ...
  getByLangue(langue) {
    this.headers.append('Accept', 'application/json;charset=UTF-8');
    const options = { headers: this.headers };
    return this.http.get(this.uri + 'byLangue/' + langue , options);
  }

  getByCandidatAndLangue(candidat, langue) {
    this.headers.append('Accept', 'application/json;charset=UTF-8');
    const options = { headers: this.headers };
    return this.http.get(this.uri + 'byCandidatFormation/' + candidat + '/' + langue , options);
  }
